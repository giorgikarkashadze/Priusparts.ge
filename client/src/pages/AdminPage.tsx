...
import {
  ...
  TrendingUp, AlertTriangle, CheckCircle, Search, X
} from 'lucide-react'
...
const labelSt: React.CSSProperties = {
  fontSize: 12, fontWeight: 500, color: '#64748b', marginBottom: 6, display: 'block'
}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 12 }}>
        {CARDS.map(({ label, value, icon: Icon, color, bg }) => (
          <div key={label} style={{ ...card, padding: 20 }}>
            ...
            <div style={{ fontSize: 26, fontWeight: 700, color: '#f9fafb', marginBottom: 2 }}>{value}</div>
            <div style={{ fontSize: 12, color: '#475569' }}>{label}</div>
          </div>
        ))}
      </div>
      <div style={{ ...card, padding: 20 }}>
        <h2 style={{ fontWeight: 600, fontSize: 15, color: '#f9fafb', marginBottom: 6 }}>Quick start</h2>
        <p style={{ fontSize: 13, color: '#475569', lineHeight: 1.6 }}>Use the tabs above to manage Prius parts inventory, view and update orders, create discount promotions, and configure store settings.</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <button onClick={() => { setShowForm(!showForm); setEditId(null); reset() }} style={{ ...btnPrimary, marginLeft: 'auto' }}>
          {showForm ? <><X size={14} /> Cancel</> : <><Plus size={14} /> Add part</>}
        </button>
      </div>
      ...
      {showForm && (
        <div style={{ ...card, overflow: 'hidden' }}>
          ...
          <form onSubmit={handleSubmit(d => saveMutation.mutate(d))} style={{ padding: 24 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>

              {/* Part name — full width */}
              <div style={{ gridColumn: '1 / -1' }}>
                <Field label="Part name *" error={errors.name?.message}>
                  <input {...register('name')} placeholder="e.g. Bosch Front Brake Pad Set — Prius Gen 3"
                    style={getInput('name')}
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                <Field label="Description">
                  <textarea {...register('description')} rows={3} placeholder="Describe this part — compatibility, specifications, brand…"
              ...
              <Field label="OEM number" error={errors.oemNumber?.message}>
                <input {...register('oemNumber')} placeholder="e.g. 04465-47080"
                  style={{ ...getInput('oemNumber'), fontFamily: 'monospace' }}
                  onFocus={() => setFocusedField('oemNumber')} onBlur={() => setFocusedField(null)} />
              </Field>

              {/* Category */}
              <Field label="Category *" error={errors.categoryId?.message}>
                <select {...register('categoryId')} style={{ ...getInput('categoryId'), cursor: 'pointer' }}
              ...
              <Field label="Price (GEL) *" error={errors.price?.message}>
                <div style={{ position: 'relative' }}>
                  ...
                  <input {...register('price')} type="number" step="0.01" placeholder="49.99"
              ...
              <Field label="Compare price — original (optional)">
                <div style={{ position: 'relative' }}>
                  <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#475569', fontSize: 13 }}>₾</span>
                  <input {...register('comparePrice')} type="number" step="0.01" placeholder="64.99 (shows crossed out)"
                    style={{ ...getInput('comparePrice'), paddingLeft: 28 }}
                    onFocus={() => setFocusedField('comparePrice')} onBlur={() => setFocusedField(null)} />
                </div>
              </Field>
                ...
                <td style={{ padding: '14px 16px', alignItems: 'center', justifyContent: 'center', display: 'flex' }}>
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <button onClick={() => deleteMutation.mutate(part.id)}
                      style={{ width: 30, height: 30, borderRadius: 7, border: '1px solid rgba(239,68,68,0.2)', background: 'rgba(239,68,68,0.08)', color: '#f87171', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Trash2 size={12} />
                    </button>
                  </div>
                </td>
...
function PromotionsTab() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div>
        <button onClick={() => setShowForm(!showForm)} style={{ ...btnPrimary, boxShadow: '0 4px 16px rgba(212,56,13,0.25)' }}>
          {showForm ? <><X size={14} /> Cancel</> : <><Plus size={14} /> Create promotion</>}
        </button>
      </div>
        ...
        <div style={{ ...card, overflow: 'hidden' }}>
          ...
          <form onSubmit={handleSubmit(d => createMutation.mutate(d))} style={{ padding: 24, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <Field label="Promo code *" error={errors.code?.message}>
              <input {...register('code')} placeholder="SUMMER30" style={{ ...inputSt, textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600 }} />
            </Field>
            <Field label="Description">
              <input {...register('description')} placeholder="Summer sale — 30% off brakes" style={inputSt} />
            </Field>
            <Field label="Discount amount *" error={errors.discount?.message}>
              <input {...register('discount')} type="number" step="0.01" placeholder="30" style={inputSt} />
            </Field>
            <Field label="Discount type *">
              <select {...register('type')} style={{ ...inputSt, cursor: 'pointer' }}>
          ...
          <tbody>
            {Array.isArray(promos) && promos.map((promo, i) => (
              <tr key={promo.id} style={{ borderTop: i === 0 ? 'none' : '1px solid #1a2744' }}>
                ...
                <td style={{ padding: '14px 16px', color: '#64748b' }}>{promo.description ?? '—'}</td>
                <td style={{ padding: '14px 16px', color: '#64748b', fontSize: 12 }}>{promo.expiresAt ? formatDate(promo.expiresAt) : 'Never'}</td>
                <td style={{ padding: '14px 16px', color: '#64748b' }}>{promo.usageCount}{promo.usageLimit ? ` / ${promo.usageLimit}` : ''}</td>
                <td style={{ padding: '14px 16px' }}>
                  <button onClick={() => toggleMutation.mutate({ id: promo.id, isActive: !promo.isActive })}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: promo.isActive ? '#22c55e' : '#334155', display: 'flex' }}>
      ...
      <div style={{ ...card, overflow: 'hidden' }}>
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            ...
            <select defaultValue="GEL" style={{ ...inputSt, cursor: 'pointer' }}>
      ...
      <div style={{ ...card, overflow: 'hidden' }}>
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            ...
            <input type="number" defaultValue="9.99" step="0.01" style={inputSt} />
          </div>
          <div>
            <label style={labelSt}>Free shipping threshold (₾)</label>
            <input type="number" defaultValue="200" style={inputSt} />
            <p style={{ fontSize: 11, color: '#334155', marginTop: 4 }}>Orders above this amount qualify for free shipping</p>
          </div>
        </div>
      </div>
