...
export default function PartCard({ part, className }: Props) {
  ...
  return (
    <div
      className={cn(
        "card overflow-hidden group hover:-translate-y-1 transition-all duration-200 hover:shadow-lg hover:shadow-black/10 dark:hover:shadow-black/40",
        className,
      )}
    >
      {/* Thumbnail */}
      ...
      <div className="p-3">
        ...
        <Link to={`/catalog/${part.slug}`}>
          <h3 className="text-sm font-medium leading-snug mb-1 hover:text-brand transition-colors line-clamp-2">
            {part.name}
          </h3>
        </Link>
        {part.oemNumber && (
